    ...
    foreach ($routes as $route) {
        if ($route->getName())
            $routes_data['"'.$route->getName().'"'] = [
                'title'     => isset($route->getAction()['title']) ? __( $route->getAction()['title']) : null,
                ...
                if (isset($value->getAction()['sub_route']) && $value->getAction()['sub_route'] == true && isset($value->getAction()['child']) && count($value->getAction()['child'])) {
                    ...
                    if (in_array($value->getName(), $my_routes)) {
                        ...
                        echo '<li class="nav-item has-treeview mt-2  ' . $opend . '" >
                                <a href="javascript:void(0);" class="nav-link side-item p-2 '.$active.'">' . $value->getAction()['icon'] . '<p class="mx-3"> ' . __($value->getAction()['title']) . '<i class="right fas fa-angle-left"></i></p></a>
                        ...
                        foreach ($value->getAction()['child'] as $child){
                            ...
                            if (isset($routes_data['"admin.' . $child . '"']) && $routes_data['"admin.' . $child . '"']['title'] && $routes_data['"admin.' . $child . '"']['icon'])
                                echo '<li class="nav-item"><a href="' . route('admin.'.$child) . '" class="nav-link p-2 ' .$active.'">' . $routes_data['"admin.' . $child . '"']['icon'] . '<p> ' . __($routes_data['"admin.' . $child . '"']['title']) . ' </p></a></li>';
                    ...
                    if (in_array($value->getName(), $my_routes)) {
                        ...
                        echo '<li class="nav-item mt-2"><a href="' . route($value->getName()) . '" class="nav-link side-item p-2 ' . $active . '">' . $value->getAction()['icon'] . ' <p class="mx-3"> ' .__( $value->getAction()['title']) . ' </p> </a></li>';
                    }
                }
            }
        }
                        ...
                        <div class="card package bg-white shadow">
                            <div class="role-title text-white">
                                <div>
                                    ...
                                    <label for="' . $parent_class . '">' . __($value->getAction()["title"]) . '</label>
                                </div>
                            </div>';
                ...
                foreach ($value->getAction()['child'] as $key => $child) {
                    ...
                    echo '<li>
                             <div class="form-group clearfix">
                                ...
                                <label for="' . $value->getName() . $key . '"> ' . __($routes_data['"admin.' . $child . '"']['title']) . '</label>
                            </div>
